...
int main(void) {
    ...
    mqd_t mq_recv_fd = mq_open(MQ_P1_TO_P2, O_CREAT | O_RDWR, 0644, &attr);
    mqd_t mq_send_fd = mq_open(MQ_P2_TO_P1, O_CREAT | O_RDWR, 0644, &attr);
    ...
    printf("[User 2] Чат запущен. Введите 'quit' для выхода.\n");
    ...
    while (1) {
        ...
        if (pri == PRI_QUIT) {
            printf("[User 2] Собеседник завершил чат.\n");
            break;
        }
        printf("Собеседник: %s\n", buf);
